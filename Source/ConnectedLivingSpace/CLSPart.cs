      ...
      // Does the part have a CLSModule on it? If so then give the module a reference to ourselves to make its life a bit easier.
      {
        ...
        else
        {
          ...
          if (part.Modules.OfType<KerbalEVA>().Count() == 0)
          {
            ...
            if (habitable)
            {
              Log.warn("Part {0} is habitable but does not have ModuleConnectedLivingSpace defined in the config. It would be better if it did as some infomation used by CLS will not be saved in the savefile.", part.partInfo.title);
            }
            else if (navigable)
            {
              Log.warn("Part {0} is passable but does not have ModuleConnectedLivingSpace defined in the config. It would be better if it did as some infomation used by CLS will not be saved in the savefile.", part.partInfo.title);
            }
          }
        }
      }
